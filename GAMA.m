function GAMA(loadpath, savepath, ith,gama)
load(loadpath);
...
while 1
    ...
    time(i) = number_arms*i + ceil(taugama(i,gama))-ceil(taugama(1,gama));
...
mkdir([savepath '\GAMA' num2str(gama) '\']);
save([savepath '\GAMA'  num2str(gama) '\' num2str(ith) '.mat']);
end
